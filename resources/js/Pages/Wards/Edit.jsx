...
export default function WardsEdit({ ward, departments = [], heads = [] }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setIsLoading(true);
        router.put(route('wards.update', ward.id), formData, {
        ...
        <AuthenticatedLayout>
            ...
            <div className="min-h-screen bg-gradient-to-br from-cyan-100 to-cyan-50 p-8">
                <div className="mb-8 flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold text-cyan-600">Edit Ward</h1>
                </div>
                <div className="bg-white rounded-lg shadow-md p-8 max-w-2xl">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <InputLabel htmlFor="name" value="Ward Name *" />
                            <TextInput id="name" name="name" type="text" className="mt-1 block w-full" value={formData.name} onChange={handleChange} required />
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <TextInput id="floor" name="floor" type="number" className="mt-1 block w-full" value={formData.floor} onChange={handleChange} />
                                {errors.floor && <InputError message={errors.floor} />}
                            </div>
                            <div>
                                <InputLabel htmlFor="capacity" value="Capacity (beds)" />
                                <TextInput id="capacity" name="capacity" type="number" className="mt-1 block w-full" value={formData.capacity} onChange={handleChange} min="1" />
                        ...
                        <div className="flex gap-4 pt-4 border-t">
                            <PrimaryButton disabled={isLoading}>{isLoading ? 'Saving...' : 'Update Ward'}</PrimaryButton>
                            <Link href={route('wards.index')}>
