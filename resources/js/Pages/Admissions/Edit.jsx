...
export default function AdmissionEdit({ admission, patients = [], wards = [] }) {
    const [formData, setFormData] = useState({
        patient_id: admission.patient_id,
        ward_id: admission.ward_id || '',
        bed_number: admission.bed_number || '',
        expected_stay_days: admission.expected_stay_days || '',
        date_admitted: admission.date_admitted,
        date_expected_leave: admission.date_expected_leave || '',
        date_actual_leave: admission.date_actual_leave || '',
    ...
    const handleSubmit = (e) => {
        ...
        router.put(route('admissions.update', admission.id), formData, {
            ...
            <div className="min-h-screen bg-gradient-to-br from-cyan-100 to-cyan-50 p-8">
                ...
                <div className="bg-white rounded-lg shadow-md p-8 max-w-2xl">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Actual Leave Date */}
                        <div>
                            <InputLabel htmlFor="date_actual_leave" value="Actual Discharge Date (optional)" />
                            <TextInput 
                                id="date_actual_leave" 
                                name="date_actual_leave" 
                                type="date" 
                                className="mt-1 block w-full" 
                                value={formData.date_actual_leave} 
                                onChange={handleChange}
                            />
                            {errors.date_actual_leave && <InputError message={errors.date_actual_leave} />}
                        </div>
