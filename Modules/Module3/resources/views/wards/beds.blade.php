                ...
                <div class="p-4 rounded-xl border-2 
                    ...
                    <div class="mt-3 space-y-2">
                        @if($bed->status == 'Available')
                            <a href="{{ route('my-wards.beds.assign', [$wardNumber, $bed->bedNumber]) }}"
                        ...
                        @if($bed->patient_name)
                            <form action="{{ route('my-wards.beds.vacate', [$wardNumber, $bed->bedNumber]) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="w-full text-xs bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">
                                    Discharge
                                </button>
                            </form>
                        @endif
